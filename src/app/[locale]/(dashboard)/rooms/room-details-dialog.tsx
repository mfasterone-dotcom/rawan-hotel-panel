...
export function RoomDetailsDialog({
  ...
  // Log room ID when viewing
  useEffect(() => {
    if (open && roomId) {
      console.log('👁️ [RoomDetailsDialog] Viewing Room ID:', roomId);
    }
  }, [open, roomId]);

  // Log when room data is loaded
  useEffect(() => {
    if (room && open) {
      console.log('👁️ [RoomDetailsDialog] Room data loaded - Room ID:', room.id);
    }
  }, [room, open]);
        ...
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
            <div>
              ...
              <p className='text-lg font-semibold'>{getRoomTypeName(room.roomType)}</p>
            </div>
            <div>
          ...
          {room.priceDecreaseRanges && room.priceDecreaseRanges.length > 0 && (
            <div>
              <h3 className='text-sm font-medium text-muted-foreground mb-2'>
                Price Decrease Ranges
              </h3>
              <div className='space-y-3'>
                {room.priceDecreaseRanges.map((range, index) => (
                  <div
                    key={index}
                    className='rounded-lg border p-4 bg-muted/50'
                  >
                    ...
                    <div className='grid grid-cols-1 gap-2 md:grid-cols-3 text-sm'>
                      ...
                      <div>
                        <span className='text-muted-foreground'>Decrease Value:</span>{' '}
                        <span className='font-medium'>{range.DecreaseValue}%</span>
                      </div>
                    </div>
                  </div>
                ))}
